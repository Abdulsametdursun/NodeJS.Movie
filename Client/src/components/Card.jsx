...
const Card = ({ movie, index }) => {
  ...
  return (
    <Link
      to={`/movie/${movie.id}`}
      className="border shadow p-3 rounded-md
      hover:bg-gray-200 cursor-pointer transition 
      max-sm:flex max-sm:gap-5"
    >
      <div className="relative">
        <img
          className="rounded w-full max-w-[450px] 
          max-h-[300px] object-cover max-sm:max-h-[150px]"
        ...
        <span
          ...
          font-semibold p-1 rounded-full text-white"
        >
          {movie.rating}
        </span>
      </div>
      ...
      <div>
        <h3
          className="font-bold text-2xl sm:text-lg 
        mt-4 line-clamp-1"
        >
          {movie.title}
        </h3>
